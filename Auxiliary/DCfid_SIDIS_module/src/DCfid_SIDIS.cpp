...
// Using parameters from: Stephan's parameters on  (September 18 2020)

const std::unordered_map<int, int> DCfid_SIDIS::pid_map = {
    {11, 0}, {2212, 1}, {211, 2}, {-211, 3}, {321, 4}, {-321, 5}};

//============ XY DC FIDUCIAL CUT Parameters In-bending============//

// Meaning of the parameters index: [Part_ID][Sector][region][parameters]

const double DCfid_SIDIS::maxparams_xy_in[6][6][3][2] = {
...
const double DCfid_SIDIS::minparams_xy_in[6][6][3][2] = {
...
//============ XY DC FIDUCIAL CUT Parameters out-bending============//

const double DCfid_SIDIS::maxparams_xy_out[6][6][3][2] = {
...
const double DCfid_SIDIS::minparams_xy_out[6][6][3][2] = {
...
const double DCfid_SIDIS::maxparams_th_ph_in[6][6][3][4] = {
...
const double DCfid_SIDIS::minparams_th_ph_in[6][6][3][4] = {
...
//=======Start writing functions for Fid cuts========//

DCfid_SIDIS::DCfid_SIDIS() {

  for (int pid = 0; pid < 6; pid++) {
    for (int s = 0; s < 6; s++) {
      for (int l = 0; l < 3; l++) {
        // Define function for xy in-bending cut
        fmin_S_L_xy_in[pid][s][l] =
            new TF1(Form("fmin_S_%d_L_%d_xy_in", s, l), "[0] + [1]*x", 0, 300);
        fmin_S_L_xy_in[pid][s][l]->SetParameters(minparams_xy_in[pid][s][l][0],
                                                 minparams_xy_in[pid][s][l][1]);

        fmax_S_L_xy_in[pid][s][l] =
            new TF1(Form("fmax_S_%d_L_%d_xy_in", s, l), "[0] + [1]*x", 0, 300);
        fmax_S_L_xy_in[pid][s][l]->SetParameters(maxparams_xy_in[pid][s][l][0],
                                                 maxparams_xy_in[pid][s][l][1]);

        // Define function for xy out-bending cut
        fmin_S_L_xy_out[pid][s][l] =
            new TF1(Form("fmin_S_%d_L_%d_xy_out", s, l), "[0] + [1]*x", 0, 300);
        fmin_S_L_xy_out[pid][s][l]->SetParameters(
            minparams_xy_out[pid][s][l][0], minparams_xy_out[pid][s][l][1]);

        fmax_S_L_xy_out[pid][s][l] =
            new TF1(Form("fmax_S_%d_L_%d_xy_out", s, l), "[0] + [1]*x", 0, 300);
        fmax_S_L_xy_out[pid][s][l]->SetParameters(
            maxparams_xy_out[pid][s][l][0], maxparams_xy_out[pid][s][l][1]);

        // Define function for theta_phi in-bending cut

        fmin_S_L_ph_in[pid][s][l] =
            new TF1(Form("fmin_S_%d_L_%d_ph_in", s, l),
                    "[0] + [1]*log(x) + [2]*x + [3]*x*x", 0, 100);
        fmin_S_L_ph_in[pid][s][l]->SetParameters(
            minparams_th_ph_in[pid][s][l][0], minparams_th_ph_in[pid][s][l][1],
            minparams_th_ph_in[pid][s][l][2], minparams_th_ph_in[pid][s][l][3]);

        fmax_S_L_ph_in[pid][s][l] =
            new TF1(Form("fmax_S_%d_L_%d_ph_in", s, l),
                    "[0] + [1]*log(x) +[2]*x + [3]*x*x", 0, 100);
        fmax_S_L_ph_in[pid][s][l]->SetParameters(
            maxparams_th_ph_in[pid][s][l][0], maxparams_th_ph_in[pid][s][l][1],
            maxparams_th_ph_in[pid][s][l][2], maxparams_th_ph_in[pid][s][l][3]);
      }
    }
  }
}
...
//    for (int s = 0; s < 6; s++) {
//      for (int l = 0; l < 3; l++) {
//        delete fmin_S_L_xy_in[pid][s][l];
//        delete fmax_S_L_xy_in[pid][s][l];
//        delete fmin_S_L_xy_out[pid][s][l];
//        delete fmax_S_L_xy_out[pid][s][l];
//
//        delete fmin_S_L_ph_in[pid][s][l];
//        delete fmax_S_L_ph_in[pid][s][l];
//      }
//    }
//  }
}
...
//========XY In-dending function=============//

TF1 *DCfid_SIDIS::get_fmin_xy_in(int pid, int sector, int layer) const {
  if (sector > 0 && sector < 7 && layer > 0 && layer < 4) {
    return fmin_S_L_xy_in[pid][sector - 1][layer - 1];
  } else {
    std::cerr << "Min_in function: sector and layer information are not right: "
              << sector << " " << layer << std::endl;
    return 0;
  }
}

TF1 *DCfid_SIDIS::get_fmax_xy_in(int pid, int sector, int layer) const {
  if (sector > 0 && sector < 7 && layer > 0 && layer < 4) {
    return fmax_S_L_xy_in[pid][sector - 1][layer - 1];
  } else {
    std::cerr << "Max_in function: sector and layer information are not right: "
              << sector << " " << layer << std::endl;
    return 0;
  }
}

//======XY out-bending function ===========//
TF1 *DCfid_SIDIS::get_fmin_xy_out(int pid, int sector, int layer) const {
  if (sector > 0 && sector < 7 && layer > 0 && layer < 4) {
    return fmin_S_L_xy_out[pid][sector - 1][layer - 1];
  } else {
    std::cerr
        << "Min_out function: sector and layer information are not right: "
        << sector << " " << layer << std::endl;
    return 0;
  }
}

TF1 *DCfid_SIDIS::get_fmax_xy_out(int pid, int sector, int layer) const {
  if (sector > 0 && sector < 7 && layer > 0 && layer < 4) {
    return fmax_S_L_xy_out[pid][sector - 1][layer - 1];
  } else {
    std::cerr
        << "Max_out function: sector and layer information are not right: "
        << sector << " " << layer << std::endl;
    return 0;
  }
}

// =======Theta_Phi In-bending functions===========//
TF1 *DCfid_SIDIS::get_fmin_ph_in(int pid, int sector, int layer) const {
  if (sector > 0 && sector < 7 && layer > 0 && layer < 4) {
    return fmin_S_L_ph_in[pid][sector - 1][layer - 1];
  } else {
    std::cerr << "Min_in_th_ph function: sector and layer information are "
                 "not right: "
              << sector << " " << layer << std::endl;
    return 0;
  }
...
TF1 *DCfid_SIDIS::get_fmax_ph_in(int pid, int sector, int layer) const {
  if (sector > 0 && sector < 7 && layer > 0 && layer < 4) {
    return fmax_S_L_ph_in[pid][sector - 1][layer - 1];
  } else {
    std::cerr << "Max_in_th_ph function: sector and layer information are "
                 "not right: "
              << sector << " " << layer << std::endl;
    return 0;
  }
...
TVector3 DCfid_SIDIS::rotate(double x, double y, int sector) const {
...
// Define a function to calculate the theta and phi from, x, y,z, sector

std::pair<double, double> DCfid_SIDIS::cal_th_ph(double x, double y, double z,
                                                 int sector) const {
  ...
  return {theta_DCr, phi_DCr};
}

//=========Define a bool function for the DC-xy fiducial ======//

bool DCfid_SIDIS::DC_fid_xy_sidis(int par_pid, double x, double y, int sector,
                                  int layer, bool bending) const {

  int pid = pid_map.at(par_pid);
  ...
  if (bending) {
    double ymin = fmin_S_L_xy_in[pid][sector - 1][layer - 1]->Eval(x_new);
    double ymax = fmax_S_L_xy_in[pid][sector - 1][layer - 1]->Eval(x_new);
    return (y_new > ymin && y_new < ymax);
  } else {
    double ymin = fmin_S_L_xy_out[pid][sector - 1][layer - 1]->Eval(x_new);
    double ymax = fmax_S_L_xy_out[pid][sector - 1][layer - 1]->Eval(x_new);
    return (y_new > ymin && y_new < ymax);
  }
}

// ======Define the DC theta_phi Fiducial cuts===========//

bool DCfid_SIDIS::DC_fid_th_ph_sidis(int par_pid, double x, double y, double z,
                                     int sector, int layer,
                                     bool bending) const {

  int pid = pid_map.at(par_pid);
  auto [theta_DCr, phi_DCr] = cal_th_ph(x, y, z, sector);

  if (bending) {
    double phi_min =
        fmin_S_L_ph_in[pid][sector - 1][layer - 1]->Eval(theta_DCr);
    double phi_max =
        fmax_S_L_ph_in[pid][sector - 1][layer - 1]->Eval(theta_DCr);
    return (phi_DCr > phi_min && phi_DCr < phi_max);
  } else {
    std::cout << "Wrong setting, th_ph cut only use for hadron in-bending: "
              << std::endl;
    return false;
  }
}
